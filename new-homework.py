...
def branch_base_dir_names(repo, name, base, is_vignette, num_parts):
    ...
    Return tuple (branch_name, base_name, doc_name, auto-sequel-branch?, exercise_no)
    """

    hw_name, base_name, hdir_name = (name, base, name)
    sequel = False  # Are we building on a previous exercise/branch?
    exercise_no = None # current vignette exercise number
    ...
    return hw_name, base_name, hdir_name, sequel, exercise_no
    ...
    for template_dir in template_dirs:
        ...
        for dirpath, _, filenames in os.walk(template_dir):
            for filename in filenames:
                contents = open(os.path.join(dirpath, filename),
                                "r", encoding="utf8").read().replace("ASSIGN", safename)

                open(os.path.join(hdir_name,
                                  filename.replace("ASSIGN", safename)),
                     "w",
...
parser.add_argument("-b", "--base",
                    ...
                    "Defaults to main, the tip of the main branch. Useful if you "
...
hw_branch, base, hdir_name, sequelp, exercise_no = \
    branch_base_dir_names(repo, aname, args.base or "main", is_vignette,
                          num_parts)
...
if is_vignette:
    print("Switched to branch '{}' for '{}' part {}.\n"
          "Type 'cd {}' at the shell prompt, and you are ready to work!"
          .format(hw_branch, hdir_name, exercise_no,
                  os.path.relpath(hw_dir, cwd)),
          file=sys.stderr)
else:
